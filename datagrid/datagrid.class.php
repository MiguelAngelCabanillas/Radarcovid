...
class DataGrid
{
    ...
    function __construct($debug_mode = false, $messaging = true, $unique_prefix = ''){
        ...
        $this->arrDgMessages = array('add_success'=>'', 'add_error'=>'', 'update_success'=>'', 'update_error'=>'', 'delete_success'=>'', 'delete_error'=>'', 'clone'=>'', 'exchange'=>'', 'move'=>'');
        ...
        $this->accessKeys = array(); // Ex.: array('add'=>'a', 'create'=>'n', 'edit'=>'e', 'update'=>'u', 'cancel'=>'c', 'details'=>'v', 'delete'=>'d');
        ...
        $this->arrExportingTypes = array('csv'=>true, 'xls'=>true, 'pdf'=>true, 'xml'=>true, 'doc'=>true);
		...
		$this->arrMultirowOperations['exchange'] = array('view'=>false, 'fields'=>array());
            ...
            if($this->CheckIsError()){                
                ...
                if($this->dbType == 'PDO'){
                    ...
                    for($counter = 0; $counter < $total_columns; $counter++){                        
                        ...
                        array('header'  => str_replace('_', ' ', $meta['name']),
                            'type'      => 'label',
                            'align'     => 'left',
                            'width'     => '',
                            'wrap'      => 'wrap',
    ...
    public function Bind($show = true){
        ...
        if($this->debug){
            ...
            echo '<br /><center><label class="default_dg_label">DataGrid v'.self::$dgVersion.' | Extension: '.$this->dbType.' | PHP: '.phpversion().' | Database: '.$this->dbDriver.' | Total running time: '.round((float)$this->endTime - (float)$this->startTime, 6).' sec.</label></center><br />';
        }
    ...
    protected function FieldsValidation($mode = ''){
		...
		foreach($this->columnsEditMode as $key => $val){
			...
			if(!empty($fp_req_type)){
				...
				else if(strlen(trim($fp_req_type)) == 2){ $field_prefix = $fp_req_type.'y'; }
			}
    ...
    protected function SetEncodingOnDatabase(){
        if(!$this->forceEncodingOnDatabase || defined('_DG_DB_ENCODED'.$this->uniquePrefix)) return false;        
		switch($this->dgGetDbDriverType()){
            case 'ibase':
            case 'firebird':
			case 'oci':
    ...
    protected function PrepareFiltering(){
        ...
        if($this->filteringAllowed){
            ...
            foreach($this->arrFilterFields as $fldName => $fldValue){
                ...
                $fldValue_fields = isset($fldValue['field']) ? explode(',', $fldValue['field']) : array();
                ...
                if(isset($fldValue['show_operator']) && $fldValue['show_operator'] != false && $fldValue['show_operator'] != 'false'){
                    if(!$this->isPrinting){
                        ...
                        }else if(isset($fldValue['default_operator']) && in_array($fldValue['default_operator'], $this->arrFilteringOperators)){
                        ...
                        $output .= '<select class="'.$this->cssClass.'_dg_select" name="'.$this->uniquePrefix.'_ff_'.$filter_field_operator.'" id="'.$this->uniquePrefix.'_ff_'.$filter_field_operator.'"'.$onchange_filter_field.'>';
                    ...
                    switch($ff_type){
                        ...
                        case 'dropdownlist':
                            ...
                            if(isset($fldValue['source']) && is_array($fldValue['source'])){                                
                                ...
                                if(in_array($this->dgGetDbDriverType(), array('oci', 'oci8'))) $this->DataSourceOld($this->dbHandler, $this->sqlView); 
                            }   
    ...
    protected function DrawTabular(){
        ...
        foreach($this->columnsViewMode as $key => $val){        
            ...
            if($fp_summarize) $this->arrSummarizeColumns[$key] = array('sum_time'=>'00:00', 'sum'=>0, 'count'=>0, 'max'=>0, 'min'=>'');
        }
        ...
        $this->RowOpen('');
            ...
            if(($this->rid == -1) && ($req_mode == 'add')){
                ...
                $req_sort_field    = (int)$this->GetVariableAlphaNumeric('sort_field');
                $req_sort_field_by = $this->GetVariableAlphaNumeric('sort_field_by');
                $req_sort_type     = $this->GetVariableAlpha('sort_type');    
                if($req_sort_field){
        ...
        for($r = $this->rowLower; (($r >=0 && $this->rowUpper >=0) && ($r < $this->rowUpper) && ($r < ($this->rowLower + $this->reqPageSize))); $r++){            
            ...
            if($this->ModeAllowed('edit') || $side_column_allowed){
                if(($this->mode == 'edit') && $this->GetFieldOffset($this->primaryKey) != '-1' && (intval($this->rid) == intval($row[$this->GetFieldOffset($this->primaryKey)]))){
                    ...
                    if(in_array($this->dgGetDbDriverType(), array('oci', 'oci8', 'odbc'))){
                        $row_id = $row[0];
                    }else{
            ...
            for($c_sorted = $this->colLower; $c_sorted < $this->sortedColumnsSize; $c_sorted++){
                ...
                if(($this->mode === 'view') && ($this->CanViewField($c_field_name))){
                    ...
                    if(!in_array($fp_summarize_function, array('SUM_TIME', 'SUM', 'AVG', 'MAX', 'MIN'))) $fp_summarize_function = $this->summarizeFunction;
                    ...
                    if($fp_summarize){
                        ...
                        else if($fp_summarize_function == 'SUM' || $fp_summarize_function == 'AVG'){ $this->arrSummarizeColumns[$c_field_name]['sum'] += strip_tags($curr_value); }
                        else if($fp_summarize_function == 'MAX'){ if($curr_value > $this->arrSummarizeColumns[$c_field_name]['max']) $this->arrSummarizeColumns[$c_field_name]['max'] = $curr_value; }
    ...
    protected function DrawColumnar(){
        ...
        if($this->drawTopButtons && $this->sortedColumnsSize > 10){
            ...
            for($c_sorted = $this->colLower; $c_sorted < $this->sortedColumnsSize; $c_sorted++){
                ...
                if($this->CanViewField($c_field_name)){					
                    ...
                    if($fp_summarize){
                        ...
                        }else if($fp_summarize_function == 'SUM'){
                    ...
                    if(isset($_REQUEST) && is_array($_REQUEST)){
                        foreach($_REQUEST as $r_key => $r_val){
							...
							if($ff_val !== ''){
								...
								$ff_val = str_replace(array('(', ')', '%28', '%29', '%3C', '%3E'), '', $ff_val);
							}
    ...
    protected function AddRow(){
        ...
        if(!$unique_field_found){            
            ...
            $ind = 0;
            $max = count($_POST);
			if(in_array($this->dgGetDbDriverType(), array('oci', 'oci8'))){
                    ...
                    if($this->IsDate($fldName)){
                        ...
                        if($fldValue !== ''){
                            ...
                            if(is_array($fldValue)){    // it was a multiple enum
                                $fldValue_new = '';
                                foreach($fldValue as $val){ if(!empty($fldValue_new)) $fldValue_new .= ','; $fldValue_new .= $val; }
                            }else if($this->IsMultiSelect($fldName)){
                        ...
                        if(trim($fldValue) != ''){
                            if($this->convertOutput) $fldValue = hebrev(mb_convert_encoding($fldValue, $this->convertFrom, $this->convertTo));
                            if(in_array($this->dgGetDbDriverType(), array('oci', 'oci8'))) $sql_values .= (int)$fldValue;
    ...
    protected function GetForeignKeyInput($keyFieldValue, $fk_field_name, $fk_field_value, $mode='edit', $multirow_postfix = ''){
        ...
        }else if(isset($this->arrForeignKeys[$fk_field_name]['elements_alignment']) && (strtolower($this->arrForeignKeys[$fk_field_name]['elements_alignment']) == 'vertical')){
    ...
    protected function SetUrlStringFiltering(&$curr_url, $amp='', $encode=false){        
        ...
        foreach($this->arrFilterFields as $fldName => $fldValue){
            ...
            foreach($field_value_split as $fldValueField){
                ...
                }else if(isset($_REQUEST[$field_name_in_url]) && is_array($_REQUEST[$field_name_in_url])){
                ...
                if($mode === 'edit'){
                    ...
                    switch($field_type){
                        ...
                        case 'enum':
                            ...
                            switch($fp_view_type){
                                case 'radiobutton':
									...
									if(empty($select_array)) $select_array = array();
                                ...
                                case 'checkbox':
                                    ...
                                    if($enum_multiple && in_array($this->GetVariable('file_act'), array('upload', 'remove'))){
                                        if($enum_multiple) $field_value = $this->GetVariable($field_req_type.$field_name.$multirow_postfix, false, 'request', false, '', true);
                                    }                                
                                    if(is_array($this->columnsEditMode[$field_name]['source'])){  // don't remove columns_edit_mode
										$select_array = isset($this->columnsEditMode[$field_name]['source']) ? $this->columnsEditMode[$field_name]['source'] : array();
										...
										$select_array = $this->GetEnumValues($this->tblName, $field_name);
                                        $ret_enum .= $this->nbsp.$this->DrawCheckBoxes($full_field_name, '', $select_array, trim($field_value), '', '', '', $disabled, $on_js_event, $enum_multiple, $fp_multiple_size, $fp_elements_alignment).$this->nbsp;
                                    }
                                    break;    
                                case 'dropdownlist':
                                default:
                                    if(is_array($this->columnsEditMode[$field_name]['source'])){  // don't remove columns_edit_mode
                                        ...
                                        $enum_param = $this->GetEnumValues($this->tblName, $field_name);
                                        $ret_enum .= $this->nbsp.$this->DrawDropDownList($full_field_name, '', $enum_param, trim($field_value), $field_name, '', '', $disabled, $on_js_event, $enum_multiple, $fp_multiple_size).$this->nbsp;
                                    }
                                    break;
                            }
                            $output = $ret_enum;
                        ...
                        case 'textarea':
                            ...
                            else if(($resizable === true) || ($resizable == 'true') || ($resizable == 'both') || ($this->browserName == 'MSIE' && $resizable == 'vertical')) $field_class = ' resizable';
                            else if($resizable == 'horizontal') $field_class = ' resizable-horizontal';
                            else if($resizable == 'vertical') $field_class = ' resizable-vertical';
                            else if($resizable == 'none') $field_class = ' resizable-none';
                            
                            $texarea  = $this->nbsp.'<textarea class="'.$this->cssClass.'_dg_textarea'.$field_class.'" id="'.$full_field_name.'" name="'.$full_field_name.'" title="'.htmlspecialchars($this->GetFieldTitle($field_name)).'" rows="'.$field_rows.'" cols="'.$field_cols.'" '.$field_maxlength.' '.$field_width.' '.$readonly.' '.$on_js_event.$placeholder.'>'.trim($field_value).'</textarea>'.$this->nbsp;
    ...
    protected function GetVariable($var = '', $prefix = true, $method = 'request', $assign_var = false, $type = '', $array_convert = false){
        ...
        if($type == 'alpha' && !Helper::IsAlpha($output)){
            $output = '';
        }else if($type == 'int' && !Helper::IsInteger($output)){
            $output = '';
        }else if($type == 'alphanumeric' && !Helper::IsAlphaNumeric($output)){
            $output = '';
        }else{
        ...
        if(!$this->isPrinting){
            if(is_array($select_array)){
                ...
                foreach($select_array as $key => $val){
                    ...
                    else if($multiple && ($checkboxes_count % $multiple_size == 0)) $text .= '<br />';
                }
            }
        }else{
            $text = $compare;     
...
';
    ...
    protected function GetDateFormatForFilteringCal($datetime_format = ''){
        ...
        else if($datetime_format == 'datemdy')  $format = 'mm'.$this->dtSeparator.'dd'.$this->dtSeparator.'yyyy'; 
        else if($datetime_format == 'datetime') $format = 'yyyy'.$this->dtSeparator.'mm'.$this->dtSeparator.'dd H:i:s'; 
    ...
    protected function DrawCalendarButton($field_name, $field_type, $datetime_format='Y-m-d', $field_value='', $field_width='', $field_maxlength='', $on_js_event='', $readonly=false, $fp_calendar_type = 'popup', $multirow_postfix = ''){
        ...
        if($fp_calendar_type == 'floating'){
            ...
            $arr_ret_date = array();
            if($show_date){
                $arr_ret_date['y'] = "<select".$readonly_property." class='".$this->cssClass."_dg_select' name='".$field_id."__nc_year' id='".$field_id."__nc_year' onChange='_dgSetCalendarDate(\"".$this->uniquePrefix."frmEditRow\", \"".$field_id."\", \"".$datetime_format."\")'><option value=''>".$this->lang['year'].'</option>'; for($i=@date("Y")-90; $i<=@date("Y")+15; $i++) { $arr_ret_date["y"] .= "<option value='".$i."'>".$i.'</option>'; }; $arr_ret_date["y"] .= '</select>';                            
        ...
        if($output != ''){
            ...
            foreach($columnsArray[$field_name] as $search_field_key => $search_field_value){
                if(substr($search_field_key, 0, 9) == 'field_key'){
                    ...
                    $field_inner = ($this->GetFieldOffset($search_field_value) != '-1') ? str_replace(array('%2F', '%3D', '%26', '%3A'), array('/', '=', '&amp;', ':'), $row[$this->GetFieldOffset($search_field_value)-$offset]) : '';
    ...
    private function dgFetchRow(&$dSet = null){
        ...
        if(empty($dSet)){
            ...
            if($this->dgGetDbDriverType() == 'ibm'){
                $row = $dSet->fetch();
            }else if($this->dgGetDbDriverType() == 'sqlsrv'){
    ...
    private function dgQuery($sql, $type = '', $msg = '', $debug_sql = ''){
        ...
        $arr_execs = array('update', 'insert', 'delete');
        if($this->dbType == 'PDO'){
            try{
        ...
        if($this->debug){
            ...
            }else if(in_array($type, $arr_execs)){
