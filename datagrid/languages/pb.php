<?php
//------------------------------------------------------------------------------
//*** Português do Brasil	pb - (ptbr) | Brazilian Portuguese	pb - (ptbr) Por Julio Formiga (form1ga@yahoo.com.br)
//------------------------------------------------------------------------------
function setLanguagePb(){ 
    $lang['='] = "=";  // "igual";
    $lang['!='] = "!="; // "not equal"; 
    $lang['>'] = ">";  // "maior";
    $lang['>='] = ">=";  // "bigger or equal";
    $lang['<'] = "<";  // "menor";
    $lang['<='] = "<=";  // "smaller or equal";            
    $lang['add'] = "Adicionar"; 
    $lang['add_new'] = "+ Adicionar novo"; 
    $lang['add_new_record'] = "Adicionar novo registro";
    $lang['add_new_record_blocked'] = "Verificação de segurança: tentativa de adicionar um novo recorde! Verifique suas configurações, a operação não é permitida!";    
    $lang['adding_operation_completed'] = "Adição realizada com sucesso!";
    $lang['adding_operation_uncompleted'] = "A adição não foi finalizada!";                                    
    $lang['alert_perform_operation'] = "Tem certeza que pretende efectuar esta operação?";
    $lang['alert_select_row'] = "Você precisa selecionar um ou mais linhas para realizar esta operação!";    
	$lang['alert_field_cannot_be_empty'] = 'Field {title} cannot be empty! Please re-enter.';
	$lang['alert_field_must_be_alphabetic'] = 'Field {title} must have alphabetic value! Please re-enter.';
	$lang['alert_field_must_be_float'] = 'Field {title} must be a float value! Please re-enter.';
	$lang['alert_field_must_be_integer'] = 'Field {title} must be an integer value! Please re-enter.';
    $lang['and'] = "e";
    $lang['any'] = "qualquer";                                                 
    $lang['ascending'] = "Crescente"; 
    $lang['back'] = "Voltar"; 
    $lang['cancel'] = "Cancelar";
    $lang['cancel_creating_new_record'] = "Tem certeza que deseja cancelar a adição de um novo registro?";
    $lang['check_all'] = "Marcar todos";
    $lang['clear'] = "Clear";
    $lang['click_to_download'] = "Clique para download";
    $lang['clone_selected'] = "Clone selecionado";
    $lang['cloning_record_blocked'] = "Verificação de segurança: tentativa de clonar um recorde! Verifique suas configurações, a operação não é permitida!";
    $lang['cloning_operation_completed'] = "A operação de clonagem concluída com sucesso!";
    $lang['cloning_operation_uncompleted'] = "A operação de clonagem incompleto!";
    $lang['create'] = "Criar"; 
    $lang['create_new_record'] = "Criar um novo registro"; 
    $lang['current'] = "atual"; 
    $lang['delete'] = "Apagar"; 
    $lang['delete_record'] = "Apagar registro";
    $lang['delete_record_blocked'] = "Verificação de Segurança: tentativa de excluir um registro! Verifique suas configurações, a operação não é permitido!";    
    $lang['delete_selected'] = "Apagar selecionados";
    $lang['delete_selected_records'] = "Você tem certeza que deseja apagar os registros selecionados?";
    $lang['delete_this_record'] = "Você tem certeza que deseja apagar este registro?";                                 
    $lang['deleting_operation_completed'] = "Registro(s) apagado(s) com secesso!";
    $lang['deleting_operation_uncompleted'] = "Registro(s) não foi(foram) apagados!";                                    
    $lang['descending'] = "Decrescente";
    $lang['details'] = "Detalhes";
    $lang['details_selected'] = "Ver selecionados";
    $lang['download'] = "Baixar";    
    $lang['edit'] = "Editar";
    $lang['edit_selected'] = "Editar selecionados";
    $lang['edit_record'] = "Editar registro"; 
    $lang['edit_selected_records'] = "Você tem certeza que deseja editar os registros selecionados?";               
    $lang['errors'] = "Erros";
	$lang['exchange_operation_completed'] = "The exchange columns operation on selected rows completed successfully!";
	$lang['exchange_operation_uncompleted'] = "The exchange columns operation on selected rows uncompleted!";
	$lang['exchange_selected'] = "Exchange columns in selected rows";
    $lang['export_to_excel'] = "Exportar para o Excel";
    $lang['export_to_pdf'] = "Exportar para o PDF";
    $lang['export_to_word'] = "Exportar para o Word";
    $lang['export_to_xml'] = "Exportar para o XML";
    $lang['export_message'] = "<label class=\"default_dg_label\">O arquivo _FILE_ está pronto. Depois que terminar de baixar,</label> <a class=\"default_dg_error_message\" href=\"javascript: window.close();\">feche esta janela</a>.";
    $lang['field'] = "Campo"; 
    $lang['field_value'] = "Valor do campo";
    $lang['file_find_error'] = "Não foi possível achar o arquivo: <b>_FILE_</b>. <br>Verifique se este arquivo existe e se você informou o caminho corretamente!";                                    
    $lang['file_opening_error'] = "Não foi possível abrir o arquivo. Verifique suas permissões.";
    $lang['file_extension_error'] = "Upload de arquivo de erro: extensão de arquivo não permitido para upload. Selecione outro arquivo.";
    $lang['file_writing_error'] = "Não foi possível escrever no arquivo. Verifique as permissões de escrita!";
    $lang['file_invalid_file_size'] = "Tamanho do arquivo inválido";
    $lang['file_uploading_error'] = "Foi encontrado um erro ao tentar enviar o arquivo, por favor tente novamente!";
    $lang['file_deleting_error'] = "Foi encontrado um erro ao tentar apagar o arquivo!";
    $lang['first'] = "primeiro";
    $lang['format'] = "Formato";
    $lang['generate'] = "Gerar";
    $lang['handle_selected_records'] = "Tem certeza de que deseja processar os registros selecionados?";
    $lang['hide_search'] = "Esconder Procura";
    $lang['item'] = "item";
    $lang['items'] = "itens";
    $lang['last'] = "passado";
    $lang['like'] = "como";
    $lang['like%'] = "como%";  // "begins with";
    $lang['%like'] = "%como";  // "ends with";
    $lang['%like%'] = "%como%";  
    $lang['loading_data'] = "lendo dados...";
    $lang['max'] = "max";
    $lang['max_number_of_records'] = "Você excedeu o número máximo de registros permitidos!";
    $lang['move_down'] = "Descer";
    $lang['move_up'] = "Mover para cima";
    $lang['move_operation_completed'] = "A operação de linhas móveis concluída com sucesso!";
    $lang['move_operation_uncompleted'] = "A operação de linhas móveis incompleto!";    
    $lang['next'] = "próximo";
    $lang['no'] = "Não";
    $lang['no_data_found'] = "Não foram encontrados dados";
    $lang['no_data_found_error'] = "Não foram encontrados dados! Por favor verifique cuidadosamente a sintaxe do seu código!<br>Ele por diferenciar maiúsculas e minúsculas ou pode ter símbolos não identificados.";
    $lang['no_image'] = "Sem imagem";
    $lang['not_like'] = "não como";
    $lang['of'] = "de";
    $lang['operation_was_already_done'] = "A operação já foi realizado com sucesso! Você não pode repetir-lo novamente.";            
    $lang['or'] = "ou";
    $lang['pages'] = "Páginas";
    $lang['page_size'] = "Nº de resultados por página"; 
    $lang['previous'] = "anterior";
    $lang['printable_view'] = "Visualizar impressão";
    $lang['print_now'] = "Imprimir";
    $lang['print_now_title'] = "Clique aqui para imprimir esta página";
    $lang['record_n'] = "Registro #";
    $lang['refresh_page'] = "Atualizar Página";    
    $lang['remove'] = "Remover";
    $lang['reset'] = "Reiniciar";
    $lang['results'] = "Resultados";
    $lang['required_fields_msg'] = "<span style='color:#cd0000'>*</span> Itens marcados com um asterisco são obrigatórios";
    $lang['search'] = "Busca";
    $lang['search_d'] = "Busca"; // (description) 
    $lang['search_type'] = "Tipo de busca"; 
    $lang['select'] = "selecionar";
    $lang['set_date'] = "Definir data";
    $lang['sort'] = "Classificar";        
    $lang['test'] = "Teste";
    $lang['total'] = "Total";
    $lang['turn_on_debug_mode'] = "Para mais informações, mude para o modo 'debug'.";
    $lang['uncheck_all'] = "Desmarcar todos";
    $lang['unhide_search'] = "Mostrar a Busca";
    $lang['unique_field_error'] = "O campo _FIELD_ permite apenas valores únicos - favor informar novamente!";            
    $lang['update'] = "Atualizar";
    $lang['update_record'] = "Atualizar registro";
    $lang['update_record_blocked'] = "Verificação de segurança: tentativa de atualizar um registro! Verifique suas configurações, a operação não é permitida!";    
    $lang['updating_operation_completed'] = "A atualização foi realizada com sucesso!";
    $lang['updating_operation_uncompleted'] = "A atualização está incompleta!";
    $lang['upload'] = "Enviar arquivo";
    $lang['uploaded_file_not_image'] = "Verificação de Segurança: tentativa de atualizar um recorde! Verifique suas configurações, a operação não é permitido!";    
    $lang['view'] = "Visualizar";
    $lang['view_details'] = "Ver detalhes";
    $lang['warnings'] = "Alertas";
    $lang['with_selected'] = "Com selecionados";
    $lang['wrong_field_name'] = "Nome de campo errado";
    $lang['wrong_parameter_error'] = "Parâmetro errado em [<b>_FIELD_</b>]: _VALUE_";
    $lang['yes'] = "Sim";

    // date-time
    $lang['day']    = "dia";
    $lang['month']  = "mês";
    $lang['year']   = "ano";
    $lang['hour']   = "horas";
    $lang['min']    = "min";
    $lang['sec']    = "sec";
    $lang['months'][1] = "Janeiro";
    $lang['months'][2] = "Fevereiro";
    $lang['months'][3] = "Março";
    $lang['months'][4] = "Abril";
    $lang['months'][5] = "Poder";
    $lang['months'][6] = "Junho";
    $lang['months'][7] = "Julho";
    $lang['months'][8] = "Agosto";
    $lang['months'][9] = "Setembro";
    $lang['months'][10] = "Outubro";
    $lang['months'][11] = "Novembro";
    $lang['months'][12] = "Dezembro";
    
    return $lang;
}
