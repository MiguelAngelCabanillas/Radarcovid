 ...
 *
 * CHANGES by ApPHP:
 * - 2017-05-05: replaced !isset() with empty() to prevent PHP 7.1 "Illegal string offset"  warning
 * - 2017-05-05: $handle made aprivate data member
 * - 2017-05-05: $query default value made array()
 */
class SqlParser {
    ...
    public static function ParseString($sqlQuery, $cleanWhitespace = true) {
        ...
        for ($t = 0; $t < $tokenCount; $t++) {
            ...
            if (in_array($tokens[$t], self::$startparens)) {
                ...
                if (in_array(strtolower($tokens[$t]), self::$querysections) && empty($handle->query[$tokens[$t]])) {
