...
## --------------------------------------------------------------------------- #
##  ApPHP DataGrid Helper Class 0.1.4 (06.11.2012)                             #
...
class Helper
{
    ...
    public static function SetBrowserDefinitions()
    {
        ...
        // remaining two tests are for Netscape
            ...
            $val = explode(' ',stristr($agent,'netscape')); $val = explode('/',$val[0]); $bd['browser'] = $val[0]; $bd['version'] = isset($val[1]) ? $val[1] : '';
        }elseif(preg_match('/mozilla/i',$agent) && !preg_match('/rv:[0-9]\.[0-9]\.[0-9]/i',$agent)){
            $val = explode(' ',stristr($agent,'mozilla')); $val = explode('/',$val[0]); $bd['browser'] = 'Netscape'; $bd['version'] = isset($val[1]) ? $val[1] : '';
        }
    ...
    public static function GetFileMimetype($file_name)
	{
        ...
        switch($file_mimetype){
            case '.doc':
            case '.rtf':
				$file_mimetype = 'doc'; break;
            case '.txt':
				$file_mimetype = 'txt'; break;
            case '.html':
            case '.htm':
				$file_mimetype = 'binary'; break;
            case '.xml':
				$file_mimetype = 'xml'; break;
            case '.csv':
            case '.xls':
				$file_mimetype = 'xls'; break;
            case '.pdf':
				$file_mimetype = 'pdf'; break;
            case '.jpg':
            case '.jpeg':
            case '.png':
            case '.gif':
            case '.bmp':
				$file_mimetype = 'image'; break;
            case '.exe':
            case '.com':
				$file_mimetype = 'binary'; break;
            case '.tar':
    ...
    public static function GetLangAbbrForCalendar($lang_name)
	{
        ...
        switch($lang_name){
			...
			case 'pt': $return_abbrv = 'pt'; break; // Portuguese    
            ...
            case 'sv': $return_abbrv = 'sv'; break; // Swedish
    ...
    /**
     * Return list of page numbers for paging dropdownlist
     *      @param $rows
     *      @param $page_curr
     *      @param $pages_total
     */
    public static function GetPageSelections($rows, $page_curr = 1, $pages_total = 1)
	{
        ...
        if($pages_total < $show_all){
            ...
            for($i = 1; $i <= $frame_start; $i++) $pages[] = $i;     
            for($i = $frame_end_num; $i <= $pages_total; $i++) $pages[] = $i;            
            $i = $frame_start;
            while($i <= $frame_end_num){
                if($i >= $page_ninus_range && $i <= $page_plus_range){
                    $i++;
                    $met_bound = true;
                }else{
                    $i += $increment;    
                    if($i > $page_ninus_range && ! $met_bound) $i = $page_ninus_range;
                }    
                if($i > 0 && $i <= $frame_end_num) $pages[] = $i;
            }    
            sort($pages);
            $pages = array_unique($pages);
        }
		foreach($pages as $k => $v) $pages_output[$v] = $v;
        return $pages_output;
    } 
	 ...
	 * 		@param $field
	 */
	public static function IsFloat($field = '', $unsigned = false)
	{
		if($unsigned){
	 ...
	 */
	public static function IsDateTime($field, $datetime_format = '')
	{
		// Fix for mdy datetime fields, when uploading/removing file
        if(in_array($datetime_format, array('m-d-Y', 'm-d-Y H:i', 'm-d-Y H:i:s'))){
			$field_parts = explode('-', substr($field, 0, 10));
			if(count($field_parts) >= 3){
				if(strlen($field_parts[2]) == 4){
					$field = $field_parts[2].'-'.$field_parts[0].'-'.$field_parts[1];	
				}else{
					$field = $field_parts[0].'-'.$field_parts[1].'-'.$field_parts[2];	
				}
			}
		}
	...
	/**
	 * Checks if array is assosiative or not
	 * 		@param $arr
	 */
	...
	/**
	 * Converts indexes array into assosiative
	 * 		@param $arr
	 */
	public static function Indexed2Assoc($select_array = array())
	{
		$temp_array = array();
